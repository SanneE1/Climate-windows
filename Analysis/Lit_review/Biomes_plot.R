...
df <- rbind(temp, prcp) %>%
  group_by(Variable) %>%
  summarise(Annual = sum(Annual, na.rm = T),
            "Growing season" = sum(Growing, na.rm = T),
            "Dormant season" = sum(Dormant, na.rm = T)) %>%
  pivot_longer(-Variable, names_to = "Type", values_to = "N_Studies") %>%
  mutate(Type = factor(Type, levels = c("Annual", "Growing season", "Dormant season")))

ggplot(df) +
  geom_bar(stat = "identity", aes(x = Variable, y = N_Studies, fill = Type), position = "dodge") +
...
ggplot(c) +
  geom_bar(stat = "identity", aes(x = Variable, y = number, fill = Category), position = position_stack(reverse = TRUE))
